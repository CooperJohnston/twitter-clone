...
import { DynamoDBDocumentClient, PutCommand, DeleteCommand, QueryCommand, GetCommand, QueryCommandInput } from "@aws-sdk/lib-dynamodb";
...
export class FollowsDB implements FollowsDAO {

  readonly tableName = "follows_v2";
...
async removeFollow(follow: Follow): Promise<void> {
  ...
  const params = {
      ...
      ReturnValues: "ALL_OLD" as const // Returns the deleted item for verification
  };
  ...
  private createFollow(follow: Follow) {
    return {
      [this.follower_handle]: follow.follower.alias,
      [this.followee_handle]: follow.followee.alias,
    }
